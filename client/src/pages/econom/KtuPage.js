...
function KtuPage(){
    ...
    const goToReader = (index) => {
        navigate(`/kturead?id=${ktuDocs[index].id}`)
    }
                ...
                {ktuDocs && ktuDocs.map((item,index) => (
                    <div key={index} className="linktoktu ogmlist_list_line bordertopnone">
                        ...
                        <div className={`icons-container`}>
                            ...
                            { (rule === 7 || store.user.account === 'superadmin') && <div style={{marginLeft:'10px'}} onClick={() => goToReader(index)} className="ogmlist_list_line_del"><i className="fa-solid fa-eye"></i></div>}
                            { (rule === 3 || store.user.account === 'superadmin') && <div onClick={() => deleteHandler(index)} style={{marginLeft:'15px'}} className="ogmlist_list_line_del"><i className="fa-solid fa-xmark"></i></div>}
                        </div>
                    </div>
                ))}
            </div>
